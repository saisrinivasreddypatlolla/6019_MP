...
public class MainActivity extends AppCompatActivity {
    ...
    public void onClickOpenAddressButton(View v) {
        ...
        Uri mapUri = Uri.parse("geo:0,0?q=" + Uri.encode(addressString));
        // COMPLETED (7) Replace the Toast with a call to showMap, passing in the Uri from the previous step
//        Uri geoLocation = Uri.parse("geo:0,0?").buildUpon().appendQueryParameter("q",addressString).build();
        showMap(this,mapUri);
    }
    ...
    public void showMap(Context context, Uri uri){
        Uri ex = Uri.parse("http://maps.google.com/maps?saddr=20.344,34.34&daddr=20.5666,45.345");
        Intent intent = new Intent(Intent.ACTION_VIEW,ex);
